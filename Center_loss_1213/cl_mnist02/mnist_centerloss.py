...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCH):
        ...
        plt.text(-7.8, 7.3, "epoch=%d" % (epoch + 1))
        plt.savefig('D:\PycharmProjects\Center_loss_1213\cl_mnist02\image01/epoch=%d.jpg' % (epoch + 1))
        plt.pause(0.1)
    plt.ioff()
    plt.show()

    # #Test
    # cnn.load_state_dict(torch.load("param/centerloss.pt"))
    # for epoch in range(EPOCH):
    #     features = []
    #     labels = []
    ...
    #     plt.ylim(ymin=-8, ymax=8)
    #     plt.text(-7.8, 7.3, "epoch=%d" % (epoch + 1))
    #     plt.savefig('D:\PycharmProjects\Center_loss_1213\cl_mnist02\image01_test/epoch=%d.jpg' % (epoch + 1))
    #     plt.pause(0.1)
    # plt.ioff()
    # plt.show()